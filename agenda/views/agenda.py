...
from agenda.serializers import AgendaCreateSerializer, AgendaListSerializer, CreateAgendasPayloadSerializer
...
class AgendaViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request: Any, *args: Any, **kwargs: Any) -> Any:
        ...
        if results and (results[0].get('modalidade') or '').upper() == 'ONLINE':
            ...
            escolhas_service = EscolhasApiService(base_url=settings.ESCOLHAS_API_URL)
            try:
                escolhas_data = escolhas_service.buscar_escolhas_por_processo_uuid(str(processo_uuid))
            except RequestException as exc:
                logger.warning('Erro ao buscar escolhas por processo_uuid=%s: %s', processo_uuid, exc)
            else:
                escolhas_lista = escolhas_data if isinstance(escolhas_data, list) else escolhas_data.get('results', escolhas_data) or []
                escolhas_candidato_uuids = {str(item.get('candidato_uuid')) for item in escolhas_lista if item.get('candidato_uuid') is not None}
                candidatos_uuids_restantes = [str(cand_uuid) for cand_uuid in candidatos_uuids_agenda if str(cand_uuid) not in escolhas_candidato_uuids]
    ...
    def create(self, request: Any, *args: Any, **kwargs: Any) -> Any:
        ...
        if candidatos_uuids:
            try:
                candidatos_service = CandidatosApiService(base_url=settings.CANDIDATOS_API_URL)
                results = candidatos_service.buscar_por_uuids_ordenado_por_ranking_escolha(uuids=candidatos_uuids, fields='uuid,ranking_escolha')
                ordered_candidatos_uuids = [str(c['uuid']) for c in results if c.get('uuid') is not None]
            except RequestException as exc:
                logger.exception('Erro ao buscar candidatos por UUIDs: %s', exc)
                return Response({'detail': 'Erro ao consultar API de candidatos.'}, status=status.HTTP_502_BAD_GATEWAY)
        cursor = 0
        agendas_criadas = []
        agendas_atualizadas = []
        for item in agendas_data:
            ...
            if uuid_provido:
                try:
                    ...
                    serializer = AgendaCreateSerializer(agenda_existente, data=agenda_item, partial=False)
        ...
        logger.info('Agendas criadas', extra={'correlation_id': get_correlation_id(), 'method': request.method, 'path': request.path, 'processo_uuid': processo_uuid, 'processo_nome': processo_nome, 'agendas_criadas': len(agendas_criadas), 'agendas_atualizadas': len(agendas_atualizadas), 'todas_agendas': len(todas_agendas), 'status_code': status_code})
    ...
    def excluir_por_processo(self, request: Any) -> Any:
        ...
        deleted, _ = Agenda.objects.filter(processo_convocacao_uuid=processo_uuid).delete()
        logger.info('Agendas excluídas por processo', extra={'correlation_id': get_correlation_id(), 'processo_uuid': processo_uuid, 'excluidas': deleted})
