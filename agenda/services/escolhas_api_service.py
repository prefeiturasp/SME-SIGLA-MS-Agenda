...
class EscolhasApiService:
    ...
    def buscar_escolhas_por_processo_uuid(self, vaga_escola__lote__processo_uuid: str) -> dict[str, Any]:
        ...
        response = http_client.get(url, params=params, headers=self._headers, timeout=self.timeout_seconds)
        response.raise_for_status()
        data = response.json()
        logger.info('Escolhas buscadas por vaga_escola__lote__processo_uuid=%s', vaga_escola__lote__processo_uuid)
