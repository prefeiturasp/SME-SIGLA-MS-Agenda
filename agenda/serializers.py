...
class AgendaItemCreateSerializer(serializers.Serializer):
    ...
    modalidade = serializers.CharField(max_length=20, required=False, allow_blank=True, allow_null=True)
    ...
    classificacao = serializers.IntegerField(required=True)  # quantidade de candidatos desta agenda
    sessao = serializers.CharField(max_length=200, required=False, allow_blank=True, allow_null=True)
...
class CreateAgendasPayloadSerializer(serializers.Serializer):
    """
    Valida o payload do create: agendas, candidatos_uuids, processo_uuid, processo_nome.
    """
    agendas = serializers.ListField(
        child=AgendaItemCreateSerializer(),
        min_length=1,
        error_messages={'required': 'O campo "agendas" é obrigatório', 'min_length': 'Envie ao menos uma agenda'}
    )
    candidatos_uuids = serializers.ListField(
        child=serializers.UUIDField(),
        allow_empty=True,
        default=list,
        error_messages={'required': 'O campo "candidatos_uuids" é obrigatório'}
    )
    processo_uuid = serializers.UUIDField(
        error_messages={'required': 'O campo "processo_uuid" é obrigatório', 'invalid': 'processo_uuid deve ser um UUID válido'}
    )
