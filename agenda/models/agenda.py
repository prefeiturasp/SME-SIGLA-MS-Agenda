...
class Agenda(BaseModel):
    ...
    processo_convocacao_nome = models.CharField(
    ...
    escolha_em = models.DateField(
        verbose_name="Data de Escolha", blank=True, null=True
    )
    nomeacao_em = models.DateField(
        verbose_name="Data de Nomeação", blank=True, null=True
    )
    classificacao = models.IntegerField(
        verbose_name="Classificação", blank=True, null=True
    )
    sessao = models.CharField(
        max_length=200, verbose_name="Sessão", blank=True, null=True
    )
    retardatario = models.BooleanField(
        verbose_name="Retardatário", default=False, blank=True, null=True
    )
    hora_convocacao_inicio = models.TimeField(
        verbose_name="Hora de Início da Convocação", blank=True, null=True
    )
    hora_convocacao_fim = models.TimeField(
        verbose_name="Hora de Fim da Convocação", blank=True, null=True
    )
