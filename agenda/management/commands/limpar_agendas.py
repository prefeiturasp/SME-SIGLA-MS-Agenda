    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        try:
            if total_agendas > 0:
                ...
                self.stdout.write(self.style.SUCCESS(f'✅ {total_agendas} agendas removidas!'))
            restantes = Agenda.objects.count()
            if restantes == 0:
                self.stdout.write(self.style.SUCCESS('✅ Tabela de agendas completamente limpa!'))
            else:
                self.stdout.write(self.style.WARNING(f'⚠️  Ainda restam {restantes} agendas.'))
