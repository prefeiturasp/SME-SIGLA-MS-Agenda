...
class Command(BaseCommand):
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        cargos_disponiveis = [{'uuid': uuid.uuid4(), 'nome': 'Professor'}, {'uuid': uuid.uuid4(), 'nome': 'Coordenador'}, {'uuid': uuid.uuid4(), 'nome': 'Diretor'}]
        agendas_criadas = []
        for _ in range(count):
            processo = random.choice(processos_disponiveis)
            cargo = random.choice(cargos_disponiveis)
            agenda = Agenda.objects.create(processo_convocacao_uuid=processo['uuid'], processo_convocacao_nome=processo['nome'], cargo_uuid=cargo['uuid'], cargo_nome=cargo['nome'], data_escolha=timezone.now() + timezone.timedelta(days=random.randint(0, 15)))  # type: ignore[misc,attr-defined]
            agendas_criadas.append(agenda)
            self.stdout.write(f'  ✓ Agenda criada: {agenda.processo_convocacao_nome} - {agenda.cargo_nome}')
        self.stdout.write(self.style.SUCCESS(f'✅ {len(agendas_criadas)} agendas criadas com sucesso!'))
