...
def test_list_agenda_online_retorna_candidatos_uuids_restantes(mock_escolhas_class: Any, client: Any) -> None:
    ...
    Agenda.objects.create(processo_convocacao_uuid=uuid.uuid4(), processo_convocacao_nome='Processo Online', cargo_uuid=uuid.uuid4(), cargo_nome='Cargo', modalidade='ONLINE', candidatos_uuids=[cand_1, cand_2, cand_3])
    mock_service = mock_escolhas_class.return_value
    mock_service.buscar_escolhas_por_processo_uuid.return_value = [{'candidato_uuid': cand_1}]
...
def test_list_agenda_online_restantes_vazio_quando_todos_escolhidos(mock_escolhas_class: Any, client: Any) -> None:
    ...
    mock_service = mock_escolhas_class.return_value
    mock_service.buscar_escolhas_por_processo_uuid.return_value = [{'candidato_uuid': cand_1}, {'candidato_uuid': cand_2}]
    response = client.get(reverse('agendas-list'))
    assert response.status_code == status.HTTP_200_OK
...
def test_list_agenda_online_aceita_escolhas_com_results(mock_escolhas_class: Any, client: Any) -> None:
    ...
    mock_service = mock_escolhas_class.return_value
    mock_service.buscar_escolhas_por_processo_uuid.return_value = {'results': [{'candidato_uuid': cand_1}], 'count': 1}
    response = client.get(reverse('agendas-list'))
    assert response.status_code == status.HTTP_200_OK
    assert response.data['candidatos_uuids_restantes'] == []
...
def test_agenda_create(mock_service_class: Any, client: Any) -> None:
    ...
    mock_service.buscar_por_uuids_ordenado_por_ranking_escolha.return_value = [{'uuid': cand_uuid_1, 'ranking_escolha': 1}, {'uuid': cand_uuid_2, 'ranking_escolha': 2}]
    url = reverse('agendas-list')
    data = {'agendas': [{'cargo_uuid': str(cargo_uuid), 'cargo_nome': 'Cargo Novo', 'cargo_codigo': '123456', 'classificacao': 2, 'data_escolha': (timezone.now() + timedelta(days=30)).isoformat()}], 'candidatos_uuids': [str(cand_uuid_1), str(cand_uuid_2)], 'processo_uuid': str(processo_uuid), 'processo_nome': 'Processo Novo'}
...
def test_agenda_create_request_exception_api_candidatos_retorna_502(mock_service_class: Any, client: Any) -> None:
    ...
    cargo_uuid = uuid.uuid4()
    mock_service = mock_service_class.return_value
    mock_service.buscar_por_uuids_ordenado_por_ranking_escolha.side_effect = RequestException('Erro de conexão')
    url = reverse('agendas-list')
    data = {'agendas': [{'cargo_uuid': str(cargo_uuid), 'cargo_nome': 'Cargo Novo', 'cargo_codigo': '123456', 'classificacao': 1, 'data_escolha': (timezone.now() + timedelta(days=30)).isoformat()}], 'candidatos_uuids': [str(cand_uuid)], 'processo_uuid': str(processo_uuid), 'processo_nome': 'Processo Novo'}
    response = client.post(url, data, format='json')
...
def test_agenda_create_com_uuid_existente_atualiza(mock_service_class: Any, client: Any) -> None:
    ...
    processo_uuid = uuid.uuid4()
    cargo_uuid = uuid.uuid4()
    agenda_existente = Agenda.objects.create(processo_convocacao_uuid=processo_uuid, processo_convocacao_nome='Processo Original', cargo_uuid=cargo_uuid, cargo_nome='Cargo Original', candidatos_uuids=[])
    url = reverse('agendas-list')
    data = {'agendas': [{'uuid': str(agenda_existente.uuid), 'cargo_uuid': str(cargo_uuid), 'cargo_nome': 'Cargo Atualizado', 'cargo_codigo': '123', 'classificacao': 0}], 'candidatos_uuids': [], 'processo_uuid': str(processo_uuid), 'processo_nome': 'Processo Atualizado'}
    response = client.post(url, data, format='json')
    assert response.status_code == status.HTTP_200_OK
    assert Agenda.objects.count() == 1
...
def test_agenda_create_com_uuid_inexistente_cria_nova(mock_service_class: Any, client: Any) -> None:
    ...
    url = reverse('agendas-list')
    data = {'agendas': [{'uuid': str(uuid_fake), 'cargo_uuid': str(cargo_uuid), 'cargo_nome': 'Cargo Novo', 'cargo_codigo': '123', 'classificacao': 0}], 'candidatos_uuids': [], 'processo_uuid': str(processo_uuid), 'processo_nome': 'Processo Novo'}
    response = client.post(url, data, format='json')
    assert response.status_code == status.HTTP_201_CREATED
    assert Agenda.objects.count() == 1
    item = Agenda.objects.first()
...
def test_excluir_por_processo_remove_apenas_agendas_do_processo(client: Any) -> None:
    ...
    cargo_a = uuid.uuid4()
    cargo_b = uuid.uuid4()
    Agenda.objects.create(processo_convocacao_uuid=processo_a, processo_convocacao_nome='Processo A', cargo_uuid=cargo_a, cargo_nome='Cargo A', data_escolha=timezone.now(), candidatos_uuids=[])
    Agenda.objects.create(processo_convocacao_uuid=processo_a, processo_convocacao_nome='Processo A', cargo_uuid=cargo_a, cargo_nome='Cargo A2', data_escolha=timezone.now(), candidatos_uuids=[])
    outra = Agenda.objects.create(processo_convocacao_uuid=processo_b, processo_convocacao_nome='Processo B', cargo_uuid=cargo_b, cargo_nome='Cargo B', data_escolha=timezone.now(), candidatos_uuids=[])
