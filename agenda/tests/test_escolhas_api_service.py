...
@patch('agenda.services.escolhas_api_service.requests.get')
def test_buscar_escolhas_por_processo_uuid_chama_url_params_headers_timeout(mock_get):
    mock_get.return_value.raise_for_status = MagicMock()
    mock_get.return_value.json.return_value = {'results': [], 'count': 0}
...
@patch('agenda.services.escolhas_api_service.requests.get')
def test_buscar_escolhas_por_processo_uuid_converte_uuid_para_str(mock_get):
    mock_get.return_value.raise_for_status = MagicMock()
    ...
    assert mock_get.call_args[1]['params']['vaga_escola__lote__processo_uuid'] == str(u)


@patch('agenda.services.escolhas_api_service.requests.get')
def test_buscar_escolhas_por_processo_uuid_raise_for_status_propaga(mock_get):
...
@patch('agenda.services.escolhas_api_service.requests.get')
def test_buscar_escolhas_por_processo_uuid_request_exception_propaga(mock_get):
